...
public class Part2 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 1000; i++) {
            String hash = getMd5(input + i);
            for (int j = 0; j < 2016; j++) {
                hash = getMd5(hash);
            }
            nextThousandHashes.add(hash);
        }
        ...
        while(true) {
            ...
            String futureHash = getMd5(input + (index + 1000));

            for (int i = 0; i < 2016; i++) {
                futureHash = getMd5(futureHash);
            }
            nextThousandHashes.add(futureHash);
            ...
            if (tripleMatcher.find()) {
                ...
                for (String nextHash : nextThousandHashes) {
                    if (nextHash.contains(quint)) {
                        System.out.println("Found a good key at index " + index);
